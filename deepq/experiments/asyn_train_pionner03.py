...
def actor(lock1, net_list, mem_queue, total_step, update_flag, actor_num):
    acparser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    acparser.add_argument('--acenv', help='environment ID', default='AsynGpw-v0')
    ...
    model = mlp([512, 512, 256], [128], dueling=bool(acargs.acdueling), layer_norm=True)  # [512, 512, 512]
    ...
    actor_inter(
                ...
                update_starts=5000,  # 至少等于trainer中learning_starts,该参数,以便随后复制网络 5000
                param_noise=False,
...
def main():
    ...
    train_process = Process(target=trainer, args=(lock1, net_list, mem_queue, total_step, update_flag))
    actor_process_01 = Process(target=actor, args=(lock1, net_list, mem_queue, total_step, update_flag, 1))
    actor_process_02 = Process(target=actor, args=(lock1, net_list, mem_queue, total_step, update_flag, 2))
    actor_process_03 = Process(target=actor, args=(lock1, net_list, mem_queue, total_step, update_flag, 3))
    actor_process_04 = Process(target=actor, args=(lock1, net_list, mem_queue, total_step, update_flag, 4))
