...
class LaserNodeThread(threading.Thread):
    def __init__(self):
        ...
        # self.sub_name = '/robot'+str(1)+'/laser/scan'
        # self.count = 0
...
def reset_node_init():
    ...
    if ac_num == 1:
        new_obstacle_state.pose.position.x = 0.0
        new_obstacle_state.pose.position.y = 0.0
    elif ac_num == 2:
        new_obstacle_state.pose.position.x = -8.0
        new_obstacle_state.pose.position.y = 5.0
    elif ac_num == 3:
        new_obstacle_state.pose.position.x = 6.0
        new_obstacle_state.pose.position.y = -8.0
    elif ac_num == 4:
        new_obstacle_state.pose.position.x = -7.0
        new_obstacle_state.pose.position.y = -7.0
...
class AsynGpw(gym.Env):
    ...
    def reset(self):
        ...
        self.done = False
        # 每50轮random一次
        if self.eps % 50 == 0 or self.eps == 1:
    ...
    def step(self, action):
        ...
        # print(self.sample_laser_data)
        # 应该把最后的位姿信息也作为限制
        if self.distance_to_goal() < 0.5 or min(self.sample_laser_data) < 0.6:  # and (self.gazebo_robot_speed < 0.1):
            self.done = True
            if self.steps <= 20:  # for debug
                print(self.sample_laser_data)
    ...
    def reward(self):
        ...
        elif min(self.sample_laser_data) < 0.6:  # 有关障碍物距离部分,原始激光数据有噪声（或其它原因,不能直接做判定,甚至达到0.2 ???）
            # print(self.sample_laser_data)
            # print("error")
